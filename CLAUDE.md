...
- **Every client must carry every key it doesn't understand-by-UI** (card fields, `attachments`, `files/` members). A save rebuilds the whole archive, so a port dropping `attachments` deletes every attached file. The golden fixture holds a card entry and an attachment precisely so such a port fails the gates.
...
- **`types.rs`** — `SecretEntry`, `CardFields`, `Attachment`, `Attachments`, `Params`, `QuestionsData`, `MasterData`, `MasterSecret`, `AskryptFile`.
...
- **`examples/gen_vectors.rs`** — writes `app/test/fixtures/vectors.json` (card + `File` entry with attachment, via `insert_sealed`). `vault_b64` is not byte-stable (stamps), so diffing proves nothing — run the gates. In `scripts/vault-js-parity.mjs`, `serialized()` is a **whitelist** (add new per-entry keys), and `group()` fails a group that ran no checks.
...
- `lib/crypto/` — Dart port (`vault`, `kdf`, `aes`, `normalize`, `translit`, `secret_entry`). Carries card keys, `attachments` and `files/` members (written with `compress = false`) though it cannot add/remove attachments.
...
- `lib/screens/` — welcome, layered unlock, entries (search/tags/hidden), entry editor (read-only attachments + save-out via `VaultIo.saveAttachment`), questions editor, passgen, `auto_lock.dart`.
