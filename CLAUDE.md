...
- **`core/src/lib.rs`** — Crypto core: encryption (AES-256-CBC), key derivation (PBKDF2/SHA-256), ZIP archive handling, serialization, and the readers and writers below. **Nothing here ever holds an attachment's bytes.** `write_archive(W: Write + Seek)` is the one writer: it writes `askrypt.json`, then streams each referenced attachment from its source — a `Sealed` file is copied in and deflated like every other member, a `Carried` one is copied across **verbatim** by `ZipWriter::raw_copy_file`, same compressed bytes, method and CRC, no inflate and no crypto at all (legal because a vault holding an attachment does not rotate its master key — `SPEC.md` rule 5 and "Master key lifetime"). `to_bytes` is the buffered convenience over it, for the server backend and the tests. `from_path` is the reader the desktop uses: it parses `askrypt.json` and *lists* the `files/` members without opening one, so a vault holding a gigabyte of attachments opens in the memory an empty one does; `from_bytes` is its buffered twin, with the `origin: None` caveat above. `seal_attachment_to_file` / `open_attachment_to_file` are the streaming crypto pair (64 KiB chunks over an incremental CBC chain, byte-identical to the one-shot `seal_attachment`/`open_attachment`, which stay as the oracle the tests check them against), and `extract_attachment` pairs the opener with the archive layout. Measured: **~4 MiB peak RSS for a 1 GiB attachment**, through attach, save, re-save and extract. **`MAX_ATTACHMENT_BYTES` is gone** — it bounded an allocation that no longer happens, and would only have stopped legitimate vaults opening; the bomb guard moved to `MAX_JSON_BYTES` (1 MiB on `askrypt.json`, the one member a reader must hold whole, matching `server/src/vaultfile.rs` and the browser port). The Dart and JS ports still inflate, so they keep their own ceiling; that is a property of a reader, not of the format. `save_to_file`/`load_from_file` are thin conveniences over `LocalFileStorage`. **Whether a write mints a new master key turns on one thing — does the vault hold an attachment** — and `master_for_write` is the whole rule: no attachments means a fresh key on every write (only the entry list is under it, and that is re-encrypted from plaintext anyway, so rotating costs one random draw and stops a key recovered from one version opening the next); at least one attachment means the key stays, because the blobs are sealed under it and ride across the save verbatim, and rotating would mean re-encrypting every attached file on every save. The **entries** decide it, not the store: a blob no entry refers to is pruned by the same write, so dropping the last reference lets the next save rotate, while a *dangling* reference keeps the key — the conservative way round, since keeping a key can never make a file unreadable and minting one can. `create` itself is unchanged and simply re-wraps what it is handed: its sixth parameter is `master: Option<&MasterSecret>`, `decrypt_with_master` hands the key back out of the same derivation that opens the vault (`decrypt` is the wrapper that drops it), and `None` mints, which is a brand-new vault. Every save of a vault with attachments — a change of questions and answers included — must pass the recovered key, so the blobs living under it are not orphaned. The caller decides rather than `create` so that the session carries on under the very key its file was written with. **`create`'s seventh parameter is `attachments: &Attachments`**, and it exists as a parameter rather than a field you assign afterwards for exactly one reason: forgetting it is then a compile error at every call site instead of a save that silently deletes every attached file. It writes only the blobs the entries being written still refer to (`retain_referenced`), which is what makes deleting an attachment shrink the vault and what makes an orphan self-healing. `seal_attachment`/`open_attachment` are the pair around `encrypt_with_aes`/`decrypt_with_aes` — no new primitive — minting a random id and a **fresh IV per encryption**, the latter load-bearing under a key that stops rotating as soon as the vault has a file. `salt0`, `salt1` and the `data` IV are still regenerated on every write, and the IV must be: AES-CBC under a repeated key *and* IV would let anyone holding two versions of a vault read off how long a prefix of the entry list went unchanged. `SPEC.md`'s "Master key lifetime" section is the normative statement. `AskryptFile::touch` (called at the end of `create`, i.e. on every save, since saves rebuild the file) stamps `params.host`/`params.updated_at` via the `current_host`/`now_utc_rfc3339` helpers (`gethostname` + `chrono`). `current_host` writes `<os>@<host name>` — the OS half comes from `current_os`, which on Linux prefers the `ID=` value out of `/etc/os-release` (`parse_os_release_id`, cached in a `OnceLock`, rejecting anything that is not `[a-z0-9._-]` since this lands unencrypted and then in a web table cell) and otherwise `std::env::consts::OS`; a missing host name yields the OS name alone, never a dangling `ubuntu@`. Contains 25+ unit tests. This is the heart of the security model.
...
`confirm.rs` (the confirmation dialog, and the reason `rfd` now draws file pickers and nothing else: three native message boxes meant the one part of the app that asked *"are you sure?"* looked like a different program on each platform. `Dialog` is the chrome — title, body, buttons, plus the Escape and Enter answers — and `overlay` `stack`s it over the *whole* window, status bar and rail included: an `opaque` scrim so no click reaches the panes underneath, wrapping an `opaque` card so a click on the card does not fall through and answer the question. `Kind` is the shell's own vocabulary (`Quit`, `UnsavedChanges(PendingAction)`, `DeleteEntry`), and `follow.rs` builds its own `Dialog` through the same chrome, so nothing about following is restated. Four rules carry it: **Escape and the backdrop always answer Cancel**, which is always the choice that changes nothing (`Dialog::cancel` is a builder of its own so this cannot be got wrong per-dialog); **Enter is bound only where the affirmative is safe** — Delete and *Save mine* have no `on_enter`, which is the one deliberate departure from the native boxes, since those always default to the affirmative; **raising one unfocuses the tree** via `operation::focus(confirm::NO_FOCUS)`, an id no widget carries, because `focusable::focus` unfocuses everything that is not its target and iced 0.14 ships no `Task` wrapper for the bare `unfocus` — without it a `text_input` underneath keeps focus and swallows Escape; and **`resolve_confirm` `take`s the question before acting**, so a resolution that raises the next one (Quit falls through to `ExitApp` and so to `guard`) is not overwritten by the one being retired. The prompt is therefore **asynchronous** where `rfd`'s was blocking: `guard` stores the question and returns, and the answer arrives as `Message::Confirm`. `App.confirm` is cleared by `set_pane` and `clear_secret_panes`, since an index into an entry list that no longer exists is the bug otherwise waiting),
`follow.rs` (following the stored vault: the 60 s + window-focus probe, the pure `decide` policy, `Notice`/`Kind`, the `dialog` for the two kinds that pose a choice and the banner the shell renders above the working area for the two that only carry news. Not a pane, for `link.rs`'s reason — the vault it is about is open whichever pane is showing, so its state lives on the `Session`, in three fields `clear_messages` deliberately spares. A clean vault reloads silently, since `Unlocked` still holds every answer and needs to ask for nothing; anything unsaved — `is_modified()` **or** an open entry-editor or questions draft — asks instead, through `confirm.rs`'s chrome. Every reload writes a status line naming who saved the bytes and when, off the vault's own unencrypted stamp via `data::format_stamp`. The probe never sets `session.busy` — it runs every minute and would otherwise make the app look permanently busy — but the *reload* it may lead to does, because reading moves the backend onto the version being fetched and a save squeezed in alongside would inherit that revision and replace the other device's work with nothing ever shown. `busy` does not stop *typing*, so `App::install_reload` re-checks for unsaved work at the moment of applying, and every path that declines rolls the backend's revision back with `adopt_revision`), `link.rs` (browser sign-in: `LinkState`/`LinkPoll`/`Msg`, `update`, `abandon`, and the `waiting_card`/`sign_in_button` both the wizard and Settings render. Not a pane, because two panes start one and it outlives either; the in-flight link lives on the `Session`. Every reply is **generation-tagged** — cancelling cannot abort a request already in flight, so a late reply from a cancelled sign-in must be dropped rather than installed. The wait deliberately does **not** hold `session.busy`: that hides the very controls, Cancel among them, the user needs while waiting, and the spinner only animates while busy — so the waiting card is static. Polling uses the server's own `interval`, treats a 429 as retryable rather than terminal, and stops after 15 minutes with "open the page again" (the link itself stays valid on the server for 24 hours, so Reopen resumes the same one). The device label is `askrypt::current_host()` — the same `os@host` string the vault write stamp uses), `icon.rs` (glyph codepoints read out of the repo's own `bootstrap-icons.ttf` — note the font has no bare `plus`, only `plus-lg` — plus **`KEYWORDS`**, the table that gives an item its icon; see the list-row note below), `data.rs` (pure item helpers over `SecretEntry`: the entry types — now three, `Login`/`Card`/**`File`**, with `is_card`/`is_file` the two case-insensitive predicates the whole UI branches on — the filter (which reaches attachment *file names*, visible metadata like the cardholder, unlike the card secrets it deliberately skips), hash-tags — including `tag_key`/`same_tag`, the fold that makes `Work` and `work` **one** tag: the rail lists one row per key (first spelling seen) and `visible()` matches under it, since two rows that each show half the entries is the bug that fold fixes, and the browser viewer's picker and the mobile app's chips fold identically — `format_size`, the unencrypted write stamp, and `DATETIME_FORMAT` — the single local-time rendering, `%b %-d, %Y %H:%M`, that `format_timestamp_local` (Unix seconds) and `format_rfc3339_local` (RFC 3339 text, verbatim when unparseable) both apply, so entry stamps, the write stamp and the server vault listing read alike), and `panes/{mod,sidebar,list,detail,entry_editor,questions,passgen,settings,unlock,wizard,statusbar}.rs`. Deleting an item used to take two presses — the first armed the button, the second committed — which was a confirmation nobody could recognise as one; it is now `confirm::Kind::DeleteEntry` like everything else. Selection is an index into `session.entries`, never into the filtered view, so filtering can't invalidate it. Each list row draws `icon::item(&entry.name, &entry.url, &entry.tags, ..)`, which matches `icon::KEYWORDS` against those fields lowercased, split on everything that is not a letter or digit, and romanized through **core's own `translit`** when they are not ASCII — so *Сбербанк* reaches the same `bank` keyword *Sberbank* does. **The tags are asked first and outrank the name and the URL both** (`lookup_tags` before `lookup`): a tag is the one field the user typed in order to classify the entry, so *Tinkoff* under `#travel` is travel however much its name reads as a bank, and an unrecognizable name is exactly the case where a tag has something to say. Each tag is matched **on its own** rather than squashed in with the rest — `car` and `insurance` as two tags must not spell `carinsurance` between them — and across tags the *first* to match wins, which is the order they were typed in. Two things in that table are load-bearing. Each keyword says **where it may match**: `Any` is a substring, which is what finds `bank` inside *Sberbank* and `stackoverflow` across *Stack Overflow* now that the separators are gone; `Word` matches a whole token only, which is the *only* thing stopping `car` firing on *Oscar*, `team` on *Steam*, `ups` on *backups* and `api` on *capital* — a short keyword added as `Any` is a bug, and `every_keyword_is_lowercase_alphanumeric_and_unique` cannot catch it. And **the longest matching keyword wins**, ties going to the earlier row: that one rule, not the table order, is what makes `google` beat `mail` on *Google Mail*, `facebook` beat `book`, and `photoshop` beat both `photo` and `shop` — which is also why the brands are listed first, since *Facebook password* ties at eight characters. A name, URL and tag set that all match nothing fall back to `icon::placeholder`, the 16-glyph pool picked by hashing the **name alone** rather than actually randomized, because `view` runs every frame and a random pick would flicker (the name alone, so editing a URL cannot reshuffle a row that still matches nothing). A `Card` goes through `icon::card` instead, which falls back to `credit_card` rather than the pool — a card that matched nothing is still unmistakably a card — and a `File` is exempt entirely, since its subtitle only counts the files and the paperclip is the one thing marking it. Every codepoint in the table was read out of the font's `cmap`; an unmapped one renders as tofu rather than failing to compile, so a glyph added later must be eyeballed once in the running app (there is no `apple` glyph in this font — hence no such keyword). The root is **not** wrapped in a centering container — the panes are full-bleed.
...
4. A layered encryption scheme: first answer unlocks subsequent questions, all answers together unlock the master key, the master key encrypts the actual secrets. The master key is **rotated on every write unless the vault holds a file attachment** (`SPEC.md`, "Master key lifetime"), the exception being what keeps a save from re-encrypting every attached file.
...
- **`server/templates/` + `server/static/`** — The website's markup and its
  ...
  **pure by contract** — no DOM, no `fetch`, no globals, nothing persisted —
  ...
  Dart cores, whose `create` mints one when handed none: the key reaching it is
  always one the caller settled, either `generateMasterKey` when a vault comes
  into existence or `masterForWrite` on the way into every save — the port of
  core's `master_for_write`, which mints for an attachment-free vault and keeps
  the key for one holding a file (`SPEC.md`, "Master key lifetime"). `DEFAULT_ITERATIONS` is what a vault born here
  ...
  **`vault-open.js`** is the DOM around them and holds no crypto: pick (a stored
  ...
  re-encrypts the whole vault in the page under the same
  answers, work factor and normalization setting — which is why the answers are
  held for the life of the unlock — and under a master key `rebuild` settles
  with `masterForWrite`: a fresh one when nothing is attached, the vault's own
  when something is (`SPEC.md`, "Master key lifetime"). The page adopts that key
...
| `rfd` | Native file open/save dialogs — **pickers only**; every confirmation is `src/confirm.rs`'s own dialog, so it looks the same on all three platforms |
