...
//! storage backend.
//!
//! [`VaultLocation`] and [`ServerSession`] define the on-disk shape of
//! `settings.json` and `server_session.json`, and are kept compatible with what
//! earlier releases wrote.
//! [`AppSettings`] carries the preferences the three-pane UI exposes, every one
...
pub struct AppSettings {
    ...
    /// into `recent_vaults` because that is the shape earlier releases wrote.
    #[serde(default)]
...
mod tests {
    ...
    /// A settings file from an earlier release carries only `last_opened_file`;
    /// every preference this UI added must fall back to its default rather than
